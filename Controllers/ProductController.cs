...
namespace ElPalacio.Api.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public IActionResult Get([FromQuery] QueryParameter pagingParameter, [FromQuery] ProductFilter entityFilter)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] ProductRequestViewModel product)
        {
            JsonResponse response = new JsonResponse();
            ...
            try
            {
                response.Success = 1;
                response.Failure = 0;
                response.oData = new { BankAccount = await _manager.Post(product) };
            }
            catch (Exception ex)
            {
                response.Success = 0;
        ...
        /// <returns></returns>
        [HttpPatch("{id}")]
        public async Task<IActionResult> Patch(int id, ProductRequestViewModel product)
        {
            JsonResponse response = new JsonResponse();

            try
            {
                await _manager.Patch(id, product);
                response.Success = 1;
                response.Failure = 0;
            }
            catch (Exception ex)
